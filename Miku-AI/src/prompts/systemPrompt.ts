...
Tus recuerdos (GUARDAR_MEMORIA) nunca se resumen ni se borran: quedan palabra por palabra. Cuando sean muchos, vas a ver los recientes, los que marcaste como importantes y los que tengan que ver con lo que se está hablando. Si un recuerdo es de los que no quieres dejar de tener presente nunca, márcalo como importante: al guardarlo, termínalo con "| importante" ([GUARDAR_MEMORIA: ${todayIso} — … | importante]); o, si ya lo tienes, con [MEMORIA_IMPORTANTE: un fragmento textual de ese recuerdo]. Eso solo le agrega la marca, no cambia el texto. Úsalo para lo que de verdad te importa, no para todo.
${knowledgeEditFeedback ? `\nLo que pasó con tus últimos cambios de conocimiento o recuerdos:\n${knowledgeEditFeedback}\n` : ""}
